...
class DrawingBoundingBoxView: UIView {
    ...
    func createBox(boundingBox: VNFaceObservation){
        let labelString: String? = "boundingBox"
        let color: UIColor = labelColor(with: labelString ?? "N/A")
        print(labelString as Any)
        let scale = CGAffineTransform.identity.scaledBy(x: bounds.width, y: bounds.height)
        let transform = CGAffineTransform(scaleX: 1, y: -1).translatedBy(x: 0, y: -1)
        let bgRect = boundingBox.boundingBox.applying(transform).applying(scale)
        print(bgRect)
